 ...
 */
@Service("service2")
public class FileServiceNotOverWriteImpl implements FileService {
...
/*            while ((f = new File(location + suffix + "\\" + type + newName + "." + suffix)).exists()) {//文件名已存在
            ...
            if (!(f = new File(location + suffix + "\\" + type + name + "." + suffix)).exists()) {
                //不重复才扔进来
                upload.transferTo(f);
            }
            session.setAttribute("filename", name);
            session.setAttribute("userUploadFile", location + "txt\\" + type + name + ".txt");
